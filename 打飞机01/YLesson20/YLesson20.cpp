// YLesson20.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
				...
				for (int j = 0; j < 200; j ++ ) {
					if (n[j].islife == 1) {
						if (collide3(b[i], n[j]) == 1) {
							...
							n[j].islife = 0;
							killCount++;
							if (killCount >= 10)state = STATE_BOSS;
							break;
						}
					}
				}
...
void moveBoss() {
	...
	case 0:
		...
		if (gameTime % 5 == 0) {
			createBullet(bossX + 120, bossY + 60,-3,18, NPC, RGB(rand() % 256, rand() % 256, rand() % 256));
			createBullet(bossX + 120, bossY + 60, 0, 19, NPC, RGB(rand() % 256, rand() % 256, rand() % 256));
			createBullet(bossX + 120, bossY + 60, 3, 18, NPC, RGB(rand() % 256, rand() % 256, rand() % 256));
		}
		break;
	case 1:
		...
		if (gameTime % 1 == 0) {
			for (int i = 0; i < 20; i++) {
				createBullet(bossX + 120, bossY + 60,cos(0.314*i)*18, sin(0.314*i)*18, NPC, RGB(rand() % 256, rand() % 256, rand() % 256));
			}
		}
		break;
	case 2:
		...
		if (gameTime % 2 == 0) {
			createBullet(bossX + 120, bossY + 52, 0, -25, NPC, RGB(rand() % 256, rand() % 256, rand() % 256));
			createBullet(bossX + 120, bossY + 60, 0, -25, NPC, RGB(rand() % 256, rand() % 256, rand() % 256));
			createBullet(bossX + 120, bossY + 68, 0, -25, NPC, RGB(rand() % 256, rand() % 256, rand() % 256));
			createBullet(bossX , bossY + 52, 0, -25, NPC, RGB(rand() % 256, rand() % 256, rand() % 256));
			createBullet(bossX, bossY + 60, 0, -25, NPC, RGB(rand() % 256, rand() % 256, rand() % 256));
			createBullet(bossX, bossY + 68, 0, -25, NPC, RGB(rand() % 256, rand() % 256, rand() % 256));
		}
		break;
	default:
		break;
...
void drawGame()
{
	...
	case STATE_LOADING:
		...
		break;
	case STATE_BOSS:
		putimage(bossX, bossY, &img, SRCCOPY);
	case STATE_GAME:
		
		//putimage(300, 50, &img, SRCCOPY);
		drawBullet();
		drawNPC();
		if (isSkill == 1) {
			drawSkill();
...
int main()
{
	...
	initgraph(800, 600);
	loadimage(&img, L"QQ图片20231104190536.jpg", 150, 200);
	BeginBatchDraw();
